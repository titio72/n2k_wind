...
WindDirectionADS1115::WindDirectionADS1115() : ix_buffer_cos(0), ix_buffer_sin(0), sumCos(0), sumSin(0)
{
    memset(sinBuffer, 0, sizeof(uint16_t) * SIN_COS_BUFFER_SIZE);
    memset(cosBuffer, 0, sizeof(uint16_t) * SIN_COS_BUFFER_SIZE);
}

WindDirectionADS1115::~WindDirectionADS1115()
{
}
...
void WindDirectionADS1115::loop_micros(unsigned long now_micros, uint16_t test_cos_reading, uint16_t test_sin_reading) // this is called from an ISR every 1ms
{
    ...
    #ifndef NATIVE
    #ifdef SAMPLE_BUFFERING
    uint16_t i_sin = ads.readADC_SingleEnded(0);
    uint16_t i_cos = ads.readADC_SingleEnded(1);
...
void WindDirectionADS1115::setup()
{
    #ifndef NATIVE
    // initilize ADC
    ...
    ads.begin();
    ads.setGain(GAIN_TWO); // 1x gain   +/- 4.096V  1 bit = 0.125mV (for ADS1115)
    #endif
}

void WindDirectionADS1115::read_data(wind_data &wd, configuration &conf, unsigned long milliseconds)
{
    #ifdef SAMPLE_BUFFERING
...
void WindDirectionADS1115::apply_configuration(configuration &conf)
{
